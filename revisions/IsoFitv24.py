...
class starObj:
    def __init__(self,name,ra,ra_err,dec,dec_err,par,par_err,par_over_error,pmra,pmra_err,pmdec,pmdec_err,ra_dec_corr,ra_par_corr,ra_pmra_corr,ra_pmdec_corr,dec_par_corr,dec_pmra_corr,dec_pmdec_corr,par_pmra_corr,par_pmdec_corr,pmra_pmdec_corr,astro_n_obs,astro_n_good_obs,astro_n_bad_obs,astro_gof,astro_chi2,astro_noise,astro_noise_sig,astro_match_obs,astro_sigma5d,match_obs,g_mag,b_mag,r_mag,b_r,b_g,g_r,radvel,radvel_err,variable,teff,a_g,e_bp_rp,lum):
        #Declare instance variables
        self.name = name
        self.ra = float(ra)
        self.ra_error = float(ra_err)
        self.dec = float(dec)
        self.dec_error = float(dec_err)
        self.par = float(par)
...
def turboFilter():
    ...
    for cluster in clusterList:
        ...
        filtered,mag = pmFilter(cluster.distFiltered,cluster.name)
        print(f"pm(all): {len(filtered)}")
        
        mag = np.empty((0,2))
        
        pos_radius = 3*np.sqrt(cluster.stdev_pmra**2+cluster.stdev_pmdec**2)
        ...
        cluster.mag = mag
        print(f"full: {len(cluster.filtered)}")
...
def distFilter(cluster):
    ...
    for star in cluster.unfilteredWide:
        if np.less_equal(np.abs(star.par),cluster.mean_par + threshold*cluster.stdev_par):
...
def condense():
    ...
    for cluster in clusterList:
        ...
        if len(turnPoints) == 0:
            ...
            turnPoints.sort()
            cluster.turnPoint = turnPoints[0] - 0.05*abs(turnPoints[0])
...
def customPlot(var1,var2,starlist,square=True,color='default'):
    #imports
    import matplotlib.pyplot as plt
    
    plt.figure(f"customPlot_{var1}_{var2}")
    plt.title(f"Custom Plot {var1} {var2}")
    plt.xlabel(f"{var1}")
    plt.ylabel(f"{var2}")
    if color == 'default':    
        plt.scatter([eval(f"x.{var1}") for x in starlist],[eval(f"y.{var2}") for y in starlist],s=0.1,c='midnightblue')
    else:
        plt.scatter([eval(f"x.{var1}") for x in starlist],[eval(f"y.{var2}") for y in starlist],s=0.1,c=[eval(f"z.{color}") for z in starlist])
        plt.set_cmap('brg')
        clb = plt.colorbar()
    ...
    plt.savefig(f"SpecificPlots/pdf/Custom_Plot_{var1}_{var2}.png")
